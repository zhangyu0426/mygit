...
for idx in range(num_loop):
    ...
    str0='THEANO_FLAGS=mode=FAST_RUN,device=gpu0,cuda.root=/usr/local/cuda,floatX=float32,optimizer_including=cudnn python main_las.py vgg '
    ...
    str1='THEANO_FLAGS=mode=FAST_RUN,device=gpu1,cuda.root=/usr/local/cuda,floatX=float32,optimizer_including=cudnn python main_las.py vgg '
    str1=str1+str(groups[idx*4+1][0])+' '+str(groups[idx*4+1][1])+' '+str(groups[idx*4+1][2])+' '+str(groups[idx*4+1][3])+' '
    str1=str1+'2>&1 >log1.txt &'
    print str1
    os.system(str1)
 
    print "using the GPU-2"
    str2='THEANO_FLAGS=mode=FAST_RUN,device=gpu2,cuda.root=/usr/local/cuda,floatX=float32,optimizer_including=cudnn python main_las.py vgg '
    str2=str2+str(groups[idx*4+2][0])+' '+str(groups[idx*4+2][1])+' '+str(groups[idx*4+2][2])+' '+str(groups[idx*4+2][3])+' '
    str2=str2+'2>&1 >log2.txt &'
    print str2
    os.system(str2)

    print "using the GPU-3"
    str3='THEANO_FLAGS=mode=FAST_RUN,device=gpu3,cuda.root=/usr/local/cuda,floatX=float32,optimizer_including=cudnn python main_las.py vgg '
